 ...
 */
export function calculateQuantifiedCyberRisk(assets, vulnerabilities, incidents, controls, telemetryEvents, customWeights = DEFAULT_WEIGHTS) {
    ...
    const recentCriticalTelemetry = telemetryEvents.filter(e => e.severity === 'CRITICAL' || e.severity === 'HIGH' || e.isAnomaly);
    const threatRawScore = Math.min(100, Math.round(25 + recentCriticalTelemetry.length * 9.5));
    ...
    if (assets.length > 0) {
        const avgCrit = assets.reduce((acc, a) => {
            ...
            if (a.exposureLevel === 'INTERNET_FACING')
                factor *= 1.3;
            if (!a.mfaEnabled)
                factor *= 1.25;
            if (!a.edrActive)
    ...
    const incidentScore = Math.min(100, Math.round(activeIncidents.length * 28 + (incidents.length - activeIncidents.length) * 8));
    // 6. Environmental & Network Exposure (0 - 100)
    const internetFacingAssets = assets.filter(a => a.exposureLevel === 'INTERNET_FACING');
    const envExposureScore = Math.min(100, Math.round((internetFacingAssets.length / Math.max(1, assets.length)) * 75 + 15));
    // Normalize weights to sum to 1.0 in case user customized them
    const weightSum = customWeights.vulnerabilityExposure +
    ...
    const overallExact = vulnRawScore * nw.vulnerabilityExposure +
 ...
 */
export function assessOrganizationCyberRisk(organizationId, assets, vulnerabilities, incidents, controls, telemetryEvents, customWeights = DEFAULT_WEIGHTS) {
    const state = calculateQuantifiedCyberRisk(assets, vulnerabilities, incidents, controls, telemetryEvents, customWeights);
    return {
